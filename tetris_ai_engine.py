from engine import split_action
import random
...
class Tetris(py_environment.PyEnvironment):
    ...
    def __init__(self):
        self._action_spec = array_spec.BoundedArraySpec(
            shape=(), dtype=np.int32, minimum=0, maximum=(4 * Tetris.BOARD_WIDTH) - 1, name='action')
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(4,), dtype=np.int32, minimum=[0, 0, 0, 0],
            maximum=[(Tetris.BOARD_WIDTH*Tetris.BOARD_HEIGHT)+1, (Tetris.BOARD_HEIGHT * (Tetris.BOARD_WIDTH-1))+1, Tetris.BOARD_HEIGHT + 1, int(Tetris.BOARD_WIDTH * Tetris.BOARD_HEIGHT/2) + 1], name='observation')
    ...
    def _reset(self):
        ...
        self._new_round()
        self.score = 0
        self._state = self._get_board_props(self.board)
    ...
    def _get_board_props(self, board):
        ...
        return np.array([lines, holes, total_bumpiness, sum_height], dtype=np.int32)
    ...
    def split_action(action: int):
        rotation = action % Tetris.ROTATION_ACTION_COUNT
        column = action // Tetris.ROTATION_ACTION_COUNT
        return rotation, column

    def _step(self,action):
        '''Makes a play given a position and a rotation, returning the reward and if the game is over'''
        rotation, x = split_action(action)
        self.current_rotation = rotation

        # move starting from the middel x pos
        x_movement = 5 - x # pos -> move left, neg -> move right
        self.current_pos = [5, 0]
        ...
        if self.game_over:
            return ts.termination(self._state, score)
        else:
            return ts.transition(self._state, score, discount=0.95)
